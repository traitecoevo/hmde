...
This vignette demonstrates an interaction between errors from numerical
integration methods and MCMC sampling that produces a bimodal posterior
distribution as a result of numerical error. We stumbled across the
problem and are documenting it here, but account for it in the package
through either using analytic solutions, or numerical methods with
adaptive step sizes where analytic solutions are not available.
...
From the analytic solution we produce observations by adding measurement
...
![](here_be_dragons_files/figure-html/unnamed-chunk-4-2.png)

A note on error and precision: the same bad behaviour occurs even if you
use unrounded data with all the precision R offers and much smaller
...
that may occur with realistic precision and error.

### Implementing models and collecting posterior estimates

In this section we are going to run a sequence of sets of models. The
...
posterior can be constrained by such methods.
...
We are going to fit a finite mixture model to tell us about the
...
for both, and use the mean for rows where \$\hat{\beta\_}\_0 \>
mean(\hat{\beta\_}\_0)\$ as our starting value for the iterative process
to avoid singularities. The overall mean of the estimates works as a
threshold for extreme values because of the bimodality and distance
between clusters.
...
![](here_be_dragons_files/figure-html/unnamed-chunk-6-1.png)
...
is due to the same measurement error in the ‘observed’ data for all the
...
second mode shifts based on the step size.
...
We use scatter plots of the clusters for qualitative analysis. As the
...
such as \$\hat{\beta\_}\_0 \> 2\beta_0\$ which agree perfectly with the
cluster analysis.

The contours over the scatter plot come from data simulated from the
cluster’s multivariate normal distribution identified by the finite
mixture model.
...
We can double-check the numerical error by using an independent solver
with the same step size. We use the `deSolve` package which has an
implementation of RK4 and allows us to choose the step sizes using the
time parameter.
...
We want to look at the behaviour of the numerical method for the bad
...
Here is a figure that shows all of the estimated sizes over time for the
bad parameter combinations across different step sizes, compared to the
true values.
...
![](here_be_dragons_files/figure-html/unnamed-chunk-11-1.png) These are
indistinguishable values. If you look extremely closely you get some
deviation due to parameter bias, but that is not statistically relevant
to the process.

To demonstrate that a smaller step size to test the method is enough to
...
![](here_be_dragons_files/figure-html/unnamed-chunk-12-1.png)

Here are two plots showing the ODEs and analytic solutions with the bad
estimates compared to the true parameter values. To plot the ODEs we
...
For the purpose of the hmde package that this vignette is a part of, we
account for the pathologies in this particular model by using the
...
Butcher, John Charles. 2016. *Numerical Methods for Ordinary
Differential Equations*. Third. John Wiley & Sons.
