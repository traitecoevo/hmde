...
hmde_extract_samples <- function(model = NULL,
  ...
  estimate_list[[measurement_data]] <- hmde_extract_measurement_est(samples,
  ...
  estimate_list[[individual_data]] <- hmde_extract_individual_par_ests(samples,
                                                                       par_names[["ind_par_names"]],
                                                                       n_ind)
  estmate_list[[error_data]] <- hmde_extract_error_par_ests(samples, par_names[["error_par_names"]])
  ...
  if(!is.null(par_names[[population_pars_names]])){
    estimate_list[[population_data]] <- hmde_extract_pop_par_ests(samples, par_names[["population_pars_names"]])
  }
...
#' @keywords internal
#' @noRd
hmde_extract_individual_par_ests <- function(samples = NULL, individual_pars_names = NULL, n_ind = NULL){
  ...
  for(i in individual_pars_names){
    ...
    individual_data[[paste0(i, "_CI_lower")]] <- apply(samples[[i]], 2, quantile, probs=c(0.025))
    individual_data[[paste0(i, "_CI_upper")]] <- apply(samples[[i]], 2, quantile, probs=c(0.975))
  }
...
#' @keywords internal
#' @noRd
hmde_extract_pop_par_ests <- function(samples = NULL, population_pars_names = NULL){
  population_data <- tibble()

  #Extract mean of parameter posterior distributions
  for(i in population_pars_names){
    pop_data_temp <- tibble(par_name = i)
    pop_data_temp[["mean"]] <- apply(samples[[i]], 2, mean)
    pop_data_temp[["median"]] <- apply(samples[[i]], 2, median)
    pop_data_temp[["CI_lower"]] <- apply(samples[[i]], 2, quantile, probs=c(0.025))
    pop_data_temp[["CI_upper"]] <- apply(samples[[i]], 2, quantile, probs=c(0.975))
...
#' @keywords internal
#' @noRd
hmde_extract_error_par_ests <- function(samples = NULL, error_pars_names = NULL){
  error_data <- tibble()

  #Extract mean of parameter posterior distributions
  for(i in error_pars_names){
    error_data_temp <- tibble(par_name = i)
    error_data_temp[["mean"]] <- apply(samples[[i]], 2, mean)
    error_data_temp[["median"]] <- apply(samples[[i]], 2, median)
    CI_temp <- apply(samples[[i]], 2, quantile(probs=c(0.025,0.975)))
    error_data_temp[["CI_lower"]] <- CI_temp[,1]
    error_data_temp[["CI_upper"]] <- CI_temp[,2]

    rbind(error_data, error_data_temp)
  }
