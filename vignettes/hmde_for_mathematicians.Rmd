...
$$
For details on the prior distributions see the vignette for a specific model or check the Stan file. To see the default values run we look at the prior parameters for a given model:
```{r}
...
Estimation is currently done using MCMC through Stan. A deterministic method such as joint max posterior likelihood is intended as an option in the future.
...
We have provided a function to plot observed and estimated sizes for a selected number of individuals:
```{r}
