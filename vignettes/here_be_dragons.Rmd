...
## Load dependencies
```{r}
#remotes::install_github("traitecoevo/hmde@9633f09f158c6ee37e242bf1e3e0ae06638f722b", force = TRUE)
...
```{r}
mix_models <- list()
mix_model_plots <- list()
mix_models_par_ests <- tibble(
  good_beta_0 = c(),
  good_beta_1 = c(),
  error_beta_0 = c(),
  error_beta_1 = c(),
...
for(i in 1:length(unique(par_est_tibble$step_size))){
  ...
  mix_models[[i]] <- mvnormalmixEM(x = analysis_data[,c(3,4)], mu = mu)
  
  print(paste0("Summary of mixture model for step size ", step_size_selected))
  print(summary(mix_models[[i]]))
  
  mix_model_plots[[i]] <- plot(mix_models[[i]], 
                               whichplots = 2, 
                               xlab2 = "Beta 0", 
                               ylab2 = "Beta 1")
  
  dist_table <- tibble( #Data to calculate distance
    b_0 = c(mix_models[[i]]$mu[[2]][1], 
            mix_models[[i]]$mu[[1]][1]),
    b_1 = c(mix_models[[i]]$mu[[2]][2], 
            mix_models[[i]]$mu[[1]][2])
  )
  
  #Extract values
  mix_models_par_ests_temp <- tibble(
    good_beta_0 = mix_models[[i]]$mu[[1]][1],
    good_beta_1 = mix_models[[i]]$mu[[1]][2],
    error_beta_0 = mix_models[[i]]$mu[[2]][1],
    error_beta_1 = mix_models[[i]]$mu[[2]][2],
    step_size = step_size_selected,
    error_prob = mix_models[[i]]$lambda[2],
    dist = dist(dist_table)
  )
  
  mix_models_par_ests <- rbind(mix_models_par_ests, mix_models_par_ests_temp)
}
...
for(i in 1:nrow(legend_spec)){
  ...
  paste0("Beta_0 = ",
         signif(mix_models_par_ests$error_beta_0[i],
                digits = 3),
         ",\n Beta_1 = ",
         signif(mix_models_par_ests$error_beta_1[i], 
                             digits = 3))
  ...
  fancy_name <- paste0("Step size ", mix_models_par_ests$step_size[i], 
                       "\n", fancy_name_no_step_size)
...
for(i in 1:length(unique(par_est_tibble$step_size))){
  ...
  plot_data$good_est <- mix_models[[i]][["posterior"]][,1]
  ...
  norm_data <- as.data.frame(mvrnorm(n = 10000,
                       mu = mix_models[[i]][["mu"]][[2]],
                       Sigma = mix_models[[i]][["sigma"]][[2]]))
  ...
  scatterplot_errors_only[[i]] <- ggplot(data = error_ests_scatter, 
    ...
    annotate("text", x = xpos, y = ypos, 
             label = paste0("Probability: \n",
                            mix_models_par_ests$error_prob[i])) +
    theme_classic()
  ...
  norm_data <- as.data.frame(mvrnorm(n = 10000,
                       mu = mix_models[[i]][["mu"]][[1]],
                       Sigma = mix_models[[i]][["sigma"]][[1]]))
  names(norm_data) <- c("beta_0", "beta_1")
  ...
  scatterplot_good_only[[i]] <- ggplot(data = good_ests_scatter, 
    ...
    annotate("text", x = xpos, y = ypos, 
             label = paste0("Probability: \n",
                            (1-mix_models_par_ests$error_prob[i]))) +
      theme_classic()
}
```
...
```{r}
#Define DE function
example_DE <- function(y, beta_0, beta_1){
...
for(i in 1:nrow(mix_models_par_ests)){
  pars_combo <- c(beta_0 = mix_models_par_ests$error_beta_0[i],
                    beta_1 = mix_models_par_ests$error_beta_1[i])
  times <- seq(0, max_time, by = mix_models_par_ests$step_size[i])
  ...
  y_over_time_temp <- tibble(
    model = mix_models_par_ests$step_size[i],
    y_analytic = y_true_temp,
    y_numeric = numerical_output[,2],
    time = times,
    beta_0_par = mix_models_par_ests$error_beta_0[i],
    beta_1_par = mix_models_par_ests$error_beta_1[i]
  )
...
for(i in 1:nrow(mix_models_par_ests)){
  ...
  plot_data <- y_over_time %>%
    filter(model == mix_models_par_ests$step_size[i]) %>%
...
```{r}
#Get asymptotic size
mix_models_par_ests$Y_max <- mix_models_par_ests$error_beta_0/mix_models_par_ests$error_beta_1

#Build points for start and end of lines
plot_data <- tibble(
  x = c(0, (beta_0/beta_1), 
        rep(0, times = nrow(mix_models_par_ests)), 
        mix_models_par_ests$Y_max),
  y = c(beta_0, 0, 
        mix_models_par_ests$error_beta_0, 
        rep(0, times = nrow(mix_models_par_ests))),
  step_size = c("True pars", "True pars", 
                mix_models_par_ests$step_size, 
                mix_models_par_ests$step_size)
)
...
error_de_plot <- ggplot(data = plot_data, aes(x,y)) +
  ...
  labs(title = "ODEs for bad estimates",
       x = "Y(t)", y = "f", 
       colour = "Step size", 
       linetype = "Step size") +
  theme_classic() +
...
for(i in 1:nrow(mix_models_par_ests)){ #Add the analytic solutions
  args_list <- list(pars=c(mix_models_par_ests$error_beta_0[i],
                           mix_models_par_ests$error_beta_1[i],
                           true_y_0))
...
beta_prior_testing <- list(
  average_point <- c(mean(c(mix_models_par_ests$good_beta_0[1], 
                          mix_models_par_ests$error_beta_0[1])),
                     mean(c(mix_models_par_ests$good_beta_1[1], 
                          mix_models_par_ests$error_beta_1[1]))
  )
)
...
for(j in 1:length(beta_prior_testing)){
  prior_means <- beta_prior_testing[[j]]
