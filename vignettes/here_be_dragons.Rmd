...
for(i in 1:steps){
  ...
  y_est_temp <- tibble(
    y_hat = c(numeric_sol$k_vec*(1/6)*c(1, 2, 2, 1), numeric_sol$y_hat),
    source = c("k1", "k2", "k3", "k4", "Est. Y(t_j)")
  )
...
plot_data_all <- y_est %>%
  ...
  mutate(source = "RK4 weights k_j")
plot_data_y_hat <- y_est %>%
  filter(source == "Est. Y(t_j)") 
plot_data_Y_t <- plot_data_y_hat[seq(from = 1, to = 20, by = 2),]
plot_data_y_hat$source <- "Substep Y(t)"

plot_data <- rbind(plot_data_all, plot_data_Y_t, plot_data_y_hat)

#numeric_plot <- 
ggplot(plot_data, aes(x = index, y=y_hat, group=source)) +
  ...
  scale_colour_manual(values = c("#f8766d", "black", "darkred")) +
  scale_linewidth_manual(values = c(0.8, 0.5, 0.5)) +
  scale_size_manual(values = c(3, 2, 3)) +
  scale_shape_manual(values = c(19, 4, 3)) +
  labs(x = "Step",
       y = "Estimated Y",
       title = "Numerical estimates for step size 0.5, beta_0 = 49.4, beta_1 = 4.94",
       colour = "Source",
       shape = "Source",
       linewidth = "Source",
       size = "Source") +
