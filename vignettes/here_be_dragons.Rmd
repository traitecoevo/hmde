...
## Load dependencies
```{r}
#remotes::install_github("traitecoevo/hmde@29accb55b69bcc207e5299a84f931fe329c0d265", force = TRUE)
...
canham_obs_data <- tibble(
  y_obs = canham_true_data$y + rnorm(nrow(canham_true_data), 0, 0.1),
  time = canham_true_data$time,
  obs_index = 1:nrow(canham_true_data)
)

#Plot Canham function
...
sizes_over_time_canham <- ggplot(canham_obs_data, aes(x = time, y = y_obs)) +
  geom_point(colour = "darkorchid", size = 3) +
  geom_line(colour = "darkorchid", linewidth = 1) +
  labs(x = "Time", y = "Y") +
  theme_classic()
sizes_over_time_canham
...
for(i in 1:runs){
  ...
  suppressWarnings(
    fit <- hmde_model("canham_single_ind") |>
      hmde_assign_data(n_obs = nrow(canham_obs_data),
                       y_obs = canham_obs_data$y_obs,
                       obs_index = canham_obs_data$obs_index,
                       time = canham_obs_data$time)  |> 
      hmde_run(chains = 1, cores = 1, iter = 2000)
  )
...
canham_hist_0 <- ggplot(data = canham_par_est_tibble, aes(x=g_max)) +
  geom_histogram(fill = "lightblue",
                 colour = "black") +
  labs(x = "g_max estimates",
       title = "Parameter histograms from Canham model") +
  theme_classic()
canham_hist_1 <- ggplot(data = canham_par_est_tibble, aes(x=y_max)) +
  geom_histogram(fill = "lightgreen",
                 colour = "black") +
...
#Get some initialising estimates
possible_error <- canham_par_est_tibble %>%
  filter(g_max > mean(canham_par_est_tibble$g_max))

#To speed up the iterative algorithm we provide some initial conditions
mu <- list( #Means from true parameters and extreme estimates
  true = pars_combo,
  error = c(mean(possible_error$g_max), 
            mean(possible_error$y_max),
            mean(possible_error$k))
)

#Fit multivariate normal finite mixture model to the estimates
mix_model_canham <- mvnormalmixEM(x = canham_par_est_tibble[,3:5], mu = mu)
summary(mix_model_canham)
print(mix_model_canham$lambda)

dist_table <- tibble( #Data to calculate distance
  g_max = c(mix_model_canham$mu[[2]][1], 
            mix_model_canham$mu[[1]][1]),
  y_max = c(mix_model_canham$mu[[2]][2], 
            mix_model_canham$mu[[1]][2]),
  k = c(mix_model_canham$mu[[2]][3],
        mix_model_canham$mu[[1]][3])
)
print(dist(dist_table))
