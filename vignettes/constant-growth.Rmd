...
To change the prior parameter values (the distributions are fixed) optional arguments can be passed to `hmde_data template` with names corresponding to the `prior_pars` argument for the associated parameter as output by `hmde_models()`. For example in the following we want to change the prior for $\beta$ (`ind_beta`) in the individual model:
```{r}
...
The {hmde} workflow uses S4 class objects, starting with a data template structure that encodes the observation data and priors. You can see what data structures are needed for the constant growth model through the following methods:
...
Each name represents an element of the list that gets passed to the model: 
...
- `prior_pars_pop_log_beta_mean` is the vector of prior parameters for the log-beta mean hyper-parameter
- `prior_pars_pop_log_beta_sd` is the vector of prior parameters for the log-beta standard deviation hyper-parameter
- `prior_pars_global_error_sigma` is the vector of prior parameters for the global error standard deviation
- `model` is the name of the model

The \texttt{show}, \texttt{print} and \texttt{summary} functions are generic S4 functions implemented for the \texttt{hmde_data_template} class. There is also a generic \texttt{plot} function which plots the DE for a model with parameters taken from the priors

Now we will actually fit the model and extract the estimates. As the provided trout data is already in the form required by the `hmde_data_template` constructor function we don't need to do any further re-naming and can pass it directly.

```{r}
set.seed("hmde") #For replicable results
trout_constant_fit <- hmde_data_template("constant_multi_ind",
                                         obs_data = Trout_Size_Data) |>
  hmde_run(chains = 4, cores = 1, iter = 2000)
```

We have included two specialist diagnostic tools in the package to give easy access to the $\hat{R}$ statistics of chain convergence and mixing.
...
```
These functions exclude the $\hat{R}$ values associated with the \texttt{check_prior_pars_} generated quantites, as these have 0 variance and thus produce NAs. 

### Inspect estimates

Once the model has finished running, we can extract the model estimates and have a look at the distribution of estimated sizes, estimated growth increments, and annualised growth rates at the level of sizes over time. The \texttt{hmde_estimates} function is the constructor for the \texttt{hmde_estimtes} class. Access to estimates at a specific level use generic getters for that slot.
...
The \texttt{hmde_estimates} class also retains information on prior parameters, accessed with \texttt{prior_pars()}. 

To start our analysis, let's do some quick data wrangling to calculate our parameters of interest.
